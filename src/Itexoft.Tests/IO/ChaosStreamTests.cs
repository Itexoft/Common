...
{
    [Test]
    public void DropReadReturnsZeroAndDoesNotConsumeInnerStream()
    {
    ...
    {
        using var inner = new MemoryStream([0]);
        var options = new ChaosStreamOptions
        {
    ...
    {
        using var inner = new MemoryStream([1]);
        var options = new ChaosStreamOptions
        {
    ...
    public async Task DelaySchedulerReceivesComputedDelayAsync()
    {
        ...
        await using var chaos = new ChaosStream(inner, options);
        var buffer = new byte[1];

        var read = await chaos.ReadAsync(buffer, CancellationToken.None);
    ...
    {
        using var inner = new MemoryStream([0x00, 0x00, 0x00]);
        var options = new ChaosStreamOptions
        {
            ReadBitFlipProbability = 1,
            MaxReadBitFlips = 1,
    ...
    {
        using var inner = new MemoryStream([0x00, 0x00]);
        var options = new ChaosStreamOptions
        {
            ReadBitFlipProbability = 1,
            MaxReadBitFlips = 3,
    ...
    private sealed class ManualTimeProvider : TimeProvider
    {
        ...
        {
            Interlocked.Add(ref this.ticks, value.Ticks);
        }
    }
}