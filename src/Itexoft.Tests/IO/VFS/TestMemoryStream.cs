...
internal sealed class TestMemoryStream : Stream
{
    ...
    public TestMemoryStream(ReadOnlySpan<byte> initialData)
        : this(Math.Max(initialData.Length, 1))
    {
    ...
    public override int Read(Span<byte> destination)
    {
        lock (this.syncRoot)
        {
            ...
            if (available > 0)
            {
                var contextSuffix = context is null ? string.Empty : $" context={context}";
                DebugUtility.Log(
                    $"[TestMemoryStream] read offset={(int)this.position} len={available} first=0x{destination[0]:X2} thread={Environment.CurrentManagedThreadId}{contextSuffix}");
            }
#endif
            this.position += available;
    ...
    private void InternalWrite(ReadOnlySpan<byte> source)
    {
        ...
        if (source.Length > 0)
        {
            ...
            var contextSuffix = context is null ? string.Empty : $" context={context}";
            DebugUtility.Log(
                $"[TestMemoryStream] write offset={(int)this.position} len={source.Length} first=0x{source[0]:X2} thread={Environment.CurrentManagedThreadId}{contextSuffix}");
        }
#endif
