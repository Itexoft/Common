...
internal sealed class VirtualFileSystemStressTests
{
    ...
    public void Parallel_WritesAndReads_ShouldStayConsistent()
    {
        ...
        var baseFileCount = DebugUtility.Enabled ? Math.Min(Environment.ProcessorCount * 2, 16) : Environment.ProcessorCount * 2;
        ...
        var payloads = Enumerable.Range(0, fileCount)
            .Select(i => Enumerable.Repeat((byte)i, payloadLength).ToArray())
            .ToArray();
            ...
            i =>
            {
                ...
                if (!buffer.AsSpan().SequenceEqual(payloads[i]))
                {
                    ...
                    foreach (var extent in metadata.Extents)
                    {
                        ...
                        TestContext.WriteLine($"    tracker: {vfs.DescribeDebugTracker(extent)}");
                    }
    ...
    public void NestedVirtualFileSystems_ShouldRemainConsistent()
    {
        ...
        var baseFileCount = DebugUtility.Enabled ? Math.Min(Environment.ProcessorCount, 4) : Environment.ProcessorCount;
        var profile = CreateProfile(innermost);
        var fileCount = Math.Max(1, profile.Workload(baseFileCount));
        var payloadLength = profile.Payload(2048);
        var payloads = Enumerable.Range(0, fileCount)
            .Select(i => Enumerable.Repeat((byte)(i % 250 + 1), payloadLength).ToArray())
            .ToArray();
        ...
        Parallel.For(
            ...
            i =>
            {
                ...
                if (!buffer.AsSpan().SequenceEqual(payloads[i]))
                {
                    DebugUtility.Log($"Mismatch in nested test for {path}: expected {payloads[i][0]}, actual {buffer[0]}");
                    foreach (var entry in innermost.EnumerateDebugFileMetadata())
                        DebugUtility.Log(entry);
                    foreach (var entry in innermost.EnumerateDebugDirectory("data"))
                        DebugUtility.Log($"dir: {entry}");
                    var owners = innermost.CaptureDebugPageOwners();
                    foreach (var group in owners.OrderBy(k => k.Key))
                        DebugUtility.Log($"page {group.Key} -> file {group.Value.Value}");
                    var duplicates = innermost.FindDebugDuplicatePages();
                    foreach (var dup in duplicates)
                        DebugUtility.Log($"duplicate page {dup.Page} used by files {string.Join(",", dup.Files)}");
                    ...
                    if (fileId.IsValid)
                    {
                        ...
                        {
                            DebugUtility.Log($"file {path} extent [{extent.Start.Value}..{extent.EndExclusive})");
                    ...
                    for (var layer = 0; layer < fileSystems.Count; layer++)
                        DebugUtility.Log($"layer {layer}: {fileSystems[layer].DescribeDebugUsage()}");
                }
    ...
    public void NestedVirtualFileSystems_Sequential_Debug()
    {
        ...
        var payloads = new[]
        {
            Enumerable.Repeat((byte)1, 2048).ToArray(),
            Enumerable.Repeat((byte)2, 2048).ToArray(),
            Enumerable.Repeat((byte)3, 2048).ToArray()
        };
        ...
        var duplicatesDump = string.Join(
            "; ",
            inner.FindDebugDuplicatePages().Select(d => $"page={d.Page} owners={string.Join(',', d.Files)}"));
    ...
    public void NestedVirtualFileSystems_TwoFiles_Parallel_Debug()
    {
        ...
        var hostStreams = new List<Stream>(1);
        ...
        var payloads = new[]
        {
            Enumerable.Repeat((byte)0x11, payloadLength).ToArray(),
            Enumerable.Repeat((byte)0x22, payloadLength).ToArray()
        };

        string metadataDump;
        string usage;
        string duplicatesDump;
        ...
        metadataDump = string.Join(" | ", inner.EnumerateDebugFileMetadata());
        usage = inner.DescribeDebugUsage();
        duplicatesDump = string.Join(
            "; ",
            inner.FindDebugDuplicatePages().Select(d => $"page={d.Page} owners={string.Join(',', d.Files)}"));
        ...
        for (var index = 0; index < payloads.Length; index++)
        {
            ...
            if (!fileId.IsValid)
            {
                DebugUtility.Log($"[Debug] {path} -> invalid id");
            ...
            DebugUtility.Log($"[Debug] {path} -> {string.Join(", ", samples)}");
            pageSamples.Add($"{path} -> {string.Join(", ", samples)}");
        }

        if (mismatches.Count > 0)
            throw new(
                $"mismatches: {string.Join(", ", mismatches)} || metadata: {metadataDump} || usage: {usage} || duplicates: {duplicatesDump} || samples: {string.Join(" | ", pageSamples)}");
    }
    ...
    public void NestedVirtualFileSystems_Parallel_DebugSnapshot()
    {
        ...
        if (mismatches.Count > 0)
        {
            ...
            var duplicates = string.Join(
                "; ",
                innermost.FindDebugDuplicatePages().Select(d => $"page={d.Page} owners={string.Join(',', d.Files)}"));
            var pageSamples = new List<string>();
            ...
            throw new(
                $"mismatches: {string.Join(" | ", mismatches)} || metadata: {metadataDump} || usage: {usage} || duplicates: {duplicates} || samples: {string.Join(" | ", pageSamples)}");
        }
    }
    ...
    {
        const int layerCount = 4;
        var baseCycles = DebugUtility.Enabled ? 2 : 3;
        ...
        var baseIterationCount = DebugUtility.Enabled ? 40 : 60;
    ...
    public void CompactionEngine_ShouldHandleConcurrentWrites()
    {
        ...
        var iterations = Math.Max(1, profile.Workload(DebugUtility.Enabled ? 40 : 80));
    ...
    public void CompactionEngine_TruncateStorm_ShouldKeepData()
    {
        ...
        Parallel.For(
            ...
            worker =>
            {
                ...
                for (var iteration = 0; iteration < iterations; iteration++)
                {
                    ...
                    lastState.AddOrUpdate(
                        0,
                        newState,
                        (_, existing) => existing.Seq > newState.Seq ? existing : newState);
                    ...
                    if (iteration % 4 == 0)
                    {
                        ...
                        lastState.AddOrUpdate(
                            0,
                            cleared,
                            (_, existing) => existing.Seq > cleared.Seq ? existing : cleared);
                    }
        ...
        var expected = lastState.TryGetValue(0, out var state)
            ? state
            : (Seq: 0, Length: 0, Value: (byte)0);
    ...
    public void CrashSnapshots_ShouldRemainMountable()
    {
        ...
        void Snapshot()
        {
            snapshots.Add((memory.ToArray(), CaptureFileStates(vfs, paths)));
        }

        Snapshot();

        var iterations = DebugUtility.Enabled ? 8 : 12;
        for (var iteration = 0; iteration < iterations; iteration++)
        {
    ...
    public void NestedVirtualFileSystems_WithCompaction_ShouldStayConsistent()
    {
        ...
        var payloads = Enumerable.Range(0, fileCount)
            .Select(i => Enumerable.Repeat((byte)(i + 1), payloadLength).ToArray())
            .ToArray();
    ...
    {
        var pageSizes = new[] { 4 * 1024, PageSizing.DefaultPageSize, 1 * 1024 * 1024 };
    ...
    private sealed class ShortReadStream : Stream
    {
        ...
        public ShortReadStream(Stream inner, int maxChunk = 4096)
        {
            this.inner = inner ?? throw new ArgumentNullException(nameof(inner));
            this.maxChunk = Math.Max(1, maxChunk);
        }
