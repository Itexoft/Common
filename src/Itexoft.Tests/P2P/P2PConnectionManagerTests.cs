...
public class P2PConnectionManagerTests
{
    ...
    public async Task ConnectAsync_StubAdapter_TransitionsToEstablishedAndTracksHeartbeat()
    {
        ...
        Assert.IsTrue(
            ...
            "Manager did not return to the Established state.");
    }
    ...
    public async Task HappyEyeballs_SecondaryEndpointWinsWhenPrimaryStalls()
    {
        ...
        Assert.IsTrue(attempts.Count >= 2, "Manager did not initiate two dial attempts.");
        Assert.IsTrue(attempts.TryDequeue(out var first), "First dial attempt was not recorded.");
        Assert.IsTrue(attempts.TryDequeue(out var second), "Second dial attempt was not recorded.");
    ...
    public async Task GetMetrics_ReportsDialRateSnapshotsAndBlacklist()
    {
        ...
        Assert.IsTrue(metrics.TotalDialAttemptsLastMinute >= 2, "Unexpected number of dial attempts in metrics.");
    ...
    public async Task HappyEyeballs_RespectsMaxConcurrentDials()
    {
        ...
        Assert.IsTrue(
            ...
            "The first two attempts did not start in time.");
        Assert.AreEqual(2, Volatile.Read(ref attemptsStarted));
        Assert.IsFalse(connectTask.IsCompleted, "Connection completed before switching to the successful endpoint.");
    ...
    public async Task RateLimiter_BlacklistExpiresAndPrimaryDialResumes()
    {
        ...
        Assert.IsTrue(
            ...
            "Manager did not transition to Disconnected.");
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Disconnected, TimeSpan.FromSeconds(2)),
            "Manager did not transition to Disconnected before the third connection.");
    ...
    public async Task RateLimiter_HonorsRetryAfterFromPeer()
    {
        ...
        Assert.IsTrue(
            ...
            $"Reconnect happened too quickly: {stopwatch.Elapsed}");
    ...
    public async Task Reconnect_ParallelCallsShareSingleDial()
    {
        ...
        Task<P2PConnectionHandle>[] reconnectTasks = [manager.ReconnectAsync(), manager.ReconnectAsync()];
    ...
    public async Task DiagnosticsBroadcast_AppliesRemotePenalty()
    {
        ...
        Assert.IsTrue(
            ...
            "Manager did not apply the blacklist after receiving diagnostics.");
    ...
    public async Task SharedBlacklist_BusPropagatesAcrossManagers()
    {
        ...
        P2PTransportEndpoint[] endpoints = [new("shared.example", 9600, label: "primary", transport: "prog")];
        ...
        Assert.IsTrue(propagated, "Second manager did not receive the blacklist update via the local bus.");
        ...
        Assert.IsTrue(trackerEntry.BlacklistedUntilUtc >= observedExpiry.AddMilliseconds(-50), "Blacklist expiration was not preserved.");
    ...
    public async Task ErrorPolicy_SoftFailureUpdatesMetrics()
    {
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(5)),
            "Manager did not recover after a soft failure.");
    ...
    public async Task ErrorPolicy_HardFailureUpdatesMetrics()
    {
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(5)),
            "Manager did not recover after a hard failure.");
    ...
    public async Task TcpAdapter_RemoteResetTriggersImmediateReconnect()
    {
        ...
        Assert.IsTrue(
            ...
            "Reconnect after RST did not occur.");
        ...
        Assert.IsTrue(
            ...
            "Manager did not change the sessionId after RST.");
    ...
    public async Task NetworkEventSource_TriggersWriteProbeAndReconnect()
    {
        ...
        Assert.IsTrue(
            ...
            "Initial connection did not occur.");
        ...
        Assert.IsTrue(
            ...
            "Reconnect after the network-change signal did not occur.");
    ...
    public async Task ChaosTcpAdapter_ControlDisconnects_TriggersAutomaticReconnect()
    {
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(
                ...
                TimeSpan.FromSeconds(10)),
            "Manager did not switch to a new session.");
        ...
        Assert.IsTrue(
            await WaitForFreshHeartbeatAsync(manager, options.HeartbeatTimeout, TimeSpan.FromSeconds(5)),
            "Heartbeats did not resume after reconnect.");

        Assert.IsTrue(Volatile.Read(ref disconnects) >= 1, "Chaos injector did not trigger.");
    ...
    public async Task ChaosTcpAdapter_ReconnectUpdatesMetrics()
    {
        ...
        Assert.IsTrue(
            await WaitForFreshHeartbeatAsync(manager, options.HeartbeatTimeout, TimeSpan.FromSeconds(5)),
            "Heartbeat did not start after the first connection.");

        var metricsBefore = manager.GetMetrics();

        Assert.IsTrue(
            await WaitForConditionAsync(() => Volatile.Read(ref disconnectTriggered) == 1, TimeSpan.FromSeconds(10)),
            "Chaos injector did not drop the control channel.");
        Assert.IsTrue(
            await WaitForConditionAsync(() => Volatile.Read(ref acceptedCount) >= 2, TimeSpan.FromSeconds(15)),
            "Host did not accept the reconnection.");
        Assert.IsTrue(
            await WaitForConditionAsync(
                ...
                TimeSpan.FromSeconds(10)),
            "Manager did not switch to a new connection.");
        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(10)),
            "Manager did not return to the Established state after reconnect.");
        Assert.IsTrue(
            await WaitForFreshHeartbeatAsync(manager, options.HeartbeatTimeout, TimeSpan.FromSeconds(5)),
            "Heartbeat did not recover after reconnect.");
    ...
    public async Task ChaosTcpAdapter_HeavyTailJitterDoesNotTriggerFalsePositive()
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            Assert.IsTrue(
                ...
                "Heartbeat is not confirmed in the heavy-tail profile.");
            await Task.Delay(100);
        }

        Assert.IsTrue(
            Volatile.Read(ref unexpectedStateTransitions) == 0,
            $"Unexpected transitions detected: {string.Join(" | ", stateHistory)}");
    ...
    public async Task ChaosTcpAdapter_DataChannelDisconnectTriggersReconnect()
    {
        ...
        Assert.IsTrue(
            ...
            "Host did not accept the reconnection after the data loss.");
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(
                ...
                TimeSpan.FromSeconds(10)),
            "Manager did not switch to a new connection after the data drop.");

        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(5)),
            "Manager did not return to Established after reconnect.");

        Assert.IsTrue(
            await WaitForFreshHeartbeatAsync(manager, options.HeartbeatTimeout, TimeSpan.FromSeconds(5)),
            "Heartbeat did not recover after switching.");
    ...
    public async Task ChaosTcpAdapter_ControlAndDataDisconnectWithDelay_ReconnectsCleanly()
    {
        ...
        Assert.IsTrue(observedDataException, "Writing to the data channel did not finish with the expected exception.");
        Assert.AreEqual(1, Volatile.Read(ref dataDisconnectTriggered));

        Assert.IsTrue(
            await WaitForConditionAsync(() => Volatile.Read(ref acceptedCount) >= 2, TimeSpan.FromSeconds(15)),
            "Host did not accept the reconnection after the double drop.");
        ...
        Assert.IsTrue(
            await WaitForConditionAsync(
                ...
                TimeSpan.FromSeconds(10)),
            "Manager did not switch to a new connection after the double drop.");

        Assert.IsTrue(
            await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(10)),
            $"Manager did not return to Established after reconnect. Current state: {manager.State}. History: {string.Join("->", stateHistory)}");
    ...
    public async Task ChaosTcpAdapter_FrequentDisconnectsMaintainStateMachine()
    {
        ...
        var dataChaos = new ChaosStreamOptions
        {
            ...
            ShouldDisconnect = context =>
            {
                ...
                // Every second Write attempt tries to drop the data channel
        ...
        for (var i = 0; i < reconnectCycles; i++)
        {
            ...
            Assert.IsTrue(
                ...
                $"Host did not register reconnection #{targetDrop}. States: {string.Join(" | ", stateHistory)}");
            ...
            Assert.IsTrue(
                await WaitForConditionAsync(
                    ...
                    TimeSpan.FromSeconds(10)),
                $"Manager did not change the session after reconnect #{targetDrop}. States: {string.Join(" | ", stateHistory)}");

            previousSessionId = manager.CurrentConnection!.RemoteSessionId;

            Assert.IsTrue(
                await WaitForConditionAsync(() => manager.State == P2PConnectionState.Established, TimeSpan.FromSeconds(5)),
                $"Manager did not return to Established after reconnect #{targetDrop}.");
            ...
            Assert.IsTrue(
                await WaitForFreshHeartbeatAsync(manager, options.HeartbeatTimeout, TimeSpan.FromSeconds(5)),
                $"Heartbeat did not recover after reconnect #{targetDrop}.");
        }
    ...
    private sealed class StubTransportAdapter(string remoteNodeId) : IP2PTransportAdapter
    {
    ...
    private sealed class ProgrammableControlChannel(
        string remoteNodeId,
        Func<P2PHandshakeEnvelope, P2PHandshakeEnvelope> handshakeResponder)
        : IP2PControlChannel
    {
        ...
        private readonly Func<P2PHandshakeEnvelope, P2PHandshakeEnvelope> _handshakeResponder =
            handshakeResponder ?? throw new ArgumentNullException(nameof(handshakeResponder));
    ...
    private sealed class TrackingStream(string name) : MemoryStream
    {
    ...
    private sealed class ProgrammableTransportAdapter(
        ...
        : IP2PTransportAdapter
    {
        private readonly Func<P2PTransportEndpoint, P2PConnectionOptions, CancellationToken, Task<P2PTransportConnection>> _connectAsync =
            connectAsync ?? throw new ArgumentNullException(nameof(connectAsync));
    ...
    private sealed class ChaosTcpTransportAdapter(ChaosStreamOptions? dataOptions, ChaosStreamOptions? controlOptions)
        : IP2PTransportAdapter
    {
        private readonly ChaosStreamOptions? _controlOptions = controlOptions?.Clone();
        private readonly ChaosStreamOptions? _dataOptions = dataOptions?.Clone();
    ...
    private sealed class TestStreamControlChannel(Stream stream) : IP2PControlChannel
    {
        ...
        public async ValueTask SendAsync(P2PControlFrame frame, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    BitConverter.TryWriteBytes(span[1..], frame.TimestampUtcTicks);
                    BitConverter.TryWriteBytes(span[(1 + sizeof(long))..], frame.Sequence);
                    var written = WriteVarInt(span[HeaderSize..], payloadLength);
