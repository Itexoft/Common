...
public sealed class JsonProjectionIntegrationTests
{
    ...
    public void ProjectionCapturesDoNotBlockStringCaptures()
    {
        ...
        var plan = new JsonRewritePlanBuilder()
            .CaptureObject("/", JsonProjectionPlan<ResponseMeta>.FromConventions(), meta => rootMeta ??= meta)
            .CaptureObject("/response", JsonProjectionPlan<ResponseMeta>.FromConventions(), meta => responseMeta ??= meta)
            .CaptureValue<string>("/output/0", outputs.Add)
            .CaptureValue<string>("/output/1", outputs.Add)
            .Build();
