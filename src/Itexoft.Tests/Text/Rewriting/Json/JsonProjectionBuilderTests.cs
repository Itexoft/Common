...
public sealed class JsonProjectionBuilderTests
{
    ...
    {
        var plan = new JsonProjectionBuilder<Simple>(new() { PropertyNameStyle = JsonPropertyNameStyle.SnakeCase })
    ...
    {
        var plan = new JsonProjectionBuilder<NestedParent>(new() { PropertyNameStyle = JsonPropertyNameStyle.SnakeCase })
            .Map(x => x.Alias, x => x.Child.Value, s => int.Parse(s, CultureInfo.InvariantCulture))
            .Build();
    ...
    [TestCase(JsonPropertyNameStyle.SnakeCase, "/child/value")]
    [TestCase(JsonPropertyNameStyle.CamelCase, "/child/value")]
    ...
    {
        var plan = new JsonProjectionBuilder<NestedParent>(new() { PropertyNameStyle = style })
            .Map(x => x.Alias, x => x.Child.Value)
            .Build();
    ...
    {
        var plan = new JsonProjectionBuilder<ItemHolder>()
            .Map(x => x.ItemId, x => x.ItemId)
            .Map(x => x.ItemId, x => x.Item.Id)
            .Build();
    ...
    {
        var childPlan = new JsonProjectionBuilder<Nested>()
            .Map(x => x.Value, "/value")
            .Build();

        var parentPlan = new JsonProjectionBuilder<Parent>()
            .Map(x => x.Count, "/count", s => int.Parse(s, CultureInfo.InvariantCulture) * 2)
            .MapObject(x => x.Child, childPlan)
            .Build();
