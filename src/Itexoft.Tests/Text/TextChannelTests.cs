...
public sealed class TextChannelTests
{
    ...
    public void SyncWriterReaderRoundTripsSpanAndString()
    {
        ...
        pipe.Writer.Write(new[] { 'd', 'e', 'f' }, 0, 3);
    ...
    public async Task AsyncWriterReaderKeepsOrder()
    {
        ...
        var writerTask = Task.Run(async () =>
        {
            foreach (var ch in payload)
                await pipe.Writer.WriteAsync(ch);
        ...
        int read;
        while ((read = await pipe.Reader.ReadAsync(buffer, 0, buffer.Length)) > 0)
    ...
    public async Task BackpressureBlocksUntilReaderConsumes()
    {
        ...
        {
            await pipe.Writer.WriteAsync('A');
            await pipe.Writer.WriteAsync('B'); // should wait until 'A' consumed
            pipe.Complete();
        });
    ...
    public async Task DisposeCompletesChannel()
    {
        ...
        var read = await pipe.Reader.ReadAsync(buffer, 0, buffer.Length);
