...
internal sealed class JsonPipelineStage<THandlers>(JsonSession<THandlers> session) : IPipelineStage
{
    ...
    public ValueTask WriteAsync(ReadOnlyMemory<char> memory, CancellationToken cancellationToken)
        => this.session.WriteAsync(memory, cancellationToken);

    public void Flush() => this.session.Commit();

    public ValueTask FlushAsync(CancellationToken cancellationToken) => this.session.CommitAsync(cancellationToken);
