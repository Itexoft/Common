...
public sealed class RewritePipelineBuilder
{
    ...
    public RewritePipelineBuilder AddText<THandlers>(
        TextKernel<THandlers> kernel,
        THandlers handlers,
    ...
    public RewritePipelineBuilder AddJson<THandlers>(
        JsonKernel<THandlers> kernel,
        THandlers handlers,
        JsonKernelOptions? options = null)
        where THandlers : class
    {
        ArgumentNullException.ThrowIfNull(kernel);
