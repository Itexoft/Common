...
internal sealed class CaptureObjectRule<T> : ProjectionCaptureRule
    where T : class
{
    private readonly Action<T> onObject;
    private readonly JsonProjectionPlan<T> projection;

    internal CaptureObjectRule(string pointer, JsonProjectionPlan<T> projection, Action<T> onObject)
        : base(pointer)
    {
