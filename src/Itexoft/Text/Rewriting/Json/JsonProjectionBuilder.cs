...
public sealed class JsonProjectionBuilder<T>(JsonProjectionOptions? options = null)
    where T : class
{
        ...
        Func<string, TValue>? converter = null)
    {
        ...
        var converterFunc = converter != null
            ? new(s => converter(s)!)
            : JsonScalarConverterRegistry.Resolve(typeof(TValue));
    ...
    {
        ArgumentNullException.ThrowIfNull(property);
        ArgumentNullException.ThrowIfNull(childPlan);
        ...
        where TChild : class
    {
        ArgumentNullException.ThrowIfNull(property);
        ArgumentNullException.ThrowIfNull(childPlan);
        ArgumentNullException.ThrowIfNull(propertyPointer);
    ...
    internal void MapFromConventions()
    {
        this.MapFromConventions(new());
    }
    ...
    private void MapType(Type type, string basePointer, HashSet<Type> visited)
    {
        ...
        foreach (var member in members)
        {
            ...
            else if (JsonProjectionTypeInspector.IsObject(memberType))
            {
                ...
                method.MakeGenericMethod(memberType).Invoke(this, new object[] { member, pointer, visited });
            }
            else
            {
    ...
    private void AddObjectBindings<TChild>(MemberInfo member, string pointerBase, HashSet<Type> visited)
        where TChild : class
    {
        ArgumentNullException.ThrowIfNull(member);
        ArgumentNullException.ThrowIfNull(pointerBase);
        ArgumentNullException.ThrowIfNull(visited);
    ...
    private string BuildPointer(Expression expression)
    {
        ...
        while (true)
        {
            if (expression is MemberExpression member
                && member.Member.MemberType is MemberTypes.Property or MemberTypes.Field)
            {
