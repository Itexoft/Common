...
{
    internal static T Project<T>(string json, JsonProjectionPlan<T> plan)
        where T : class
    {
    ...
    internal static IEnumerable<T> ProjectMany<T>(string json, JsonProjectionPlan<T> plan)
        where T : class
    {
        ArgumentNullException.ThrowIfNull(json);
        ArgumentNullException.ThrowIfNull(plan);
    ...
    private static int ReadValueLength(string json, int start)
    {
        ...
        switch (ch)
        {
            ...
            case '"':
                return JsonStringReader.TryReadString(json, start, out _, out var rawLength)
                    ? rawLength
                    : 0;
            default:
    ...
    private sealed class ProjectionParser<T>(string json, JsonProjectionPlan<T> plan, T target)
        where T : class
    {
