...
public sealed class JsonRewriteWriter : TextWriter
{
    ...
    public JsonRewriteWriter(TextWriter underlying, JsonRewritePlan plan, JsonRewriteOptions? options = null)
    {
        ...
        this.enabledGroups = this.options.EnabledGroups is null
            ? null
            : new HashSet<string>(this.options.EnabledGroups, StringComparer.Ordinal);
    ...
    {
        var result = this.plan.HasAsyncRules
            ? this.ProcessBufferAsync().GetAwaiter().GetResult()
        ...
        while (i < length)
        {
            ...
            switch (ch)
            {
                ...
                default:
                    ...
                    output.Append(renderedLiteral);
                    await this.HandleValueCompletionAsync(valuePath, renderedLiteral, requirements, asyncCaptures ??= [])
    ...
    private string ApplyRename(string path, string propertyName)
    {
        ...
        for (var i = 0; i < rules.Length; i++)
        {
            ...
            if (!string.Equals(path, rule.Pointer, StringComparison.Ordinal))
                continue;
    ...
    private string RenderStringValue(string path, string value)
        => this.RenderStringValueAsync(path, value).GetAwaiter().GetResult();
        ...
        ref List<PendingAsyncCapture>? asyncCaptures)
    {
        for (var i = 0; i < requirements.Length; i++)
        {
            ...
            if (!string.Equals(path, req.Rule.Pointer, StringComparison.Ordinal))
                continue;
