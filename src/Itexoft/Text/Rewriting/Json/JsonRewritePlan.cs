...
{
    internal JsonRewritePlan(
        JsonRewriteRule[] rules,
        string?[] ruleNames,
        string?[] ruleGroups,
        string?[] ruleKinds,
        string?[] ruleTargets)
        : base(rules ?? throw new ArgumentNullException(nameof(rules)), HasAsync(rules))
    {
        this.RuleNames = ruleNames ?? Array.Empty<string?>();
        this.RuleGroups = ruleGroups ?? Array.Empty<string?>();
        this.RuleKinds = ruleKinds ?? Array.Empty<string?>();
        this.RuleTargets = ruleTargets ?? Array.Empty<string?>();
    ...
    private static TRule[] FilterByType<TRule>(JsonRewriteRule[] source)
        where TRule : JsonRewriteRule
    {
        if (source.Length == 0)
            return [];
