...
public sealed class JsonSession<THandlers> : IDisposable, IAsyncDisposable
{
    ...
    /// </summary>
    public async ValueTask WriteAsync(ReadOnlyMemory<char> chunk, CancellationToken cancellationToken = default)
    {
        this.ThrowIfDisposed();
        ...
        await this.TryEmitFramesAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    public async ValueTask CommitAsync(CancellationToken cancellationToken = default)
    {
        ...
        await this.ProcessFrameAsync(this.buffer.ToString().AsMemory(), cancellationToken).ConfigureAwait(false);
        this.buffer.Clear();
    }
    ...
    private async ValueTask TryEmitFramesAsync(CancellationToken cancellationToken)
    {
        if (this.framing is null)
            return;
        ...
        while (this.framing.TryCutFrame(ref remaining, out var frame))
        {
            ...
            await this.ProcessFrameAsync(frame, cancellationToken).ConfigureAwait(false);
        }
    ...
    private async ValueTask ProcessFrameAsync(ReadOnlyMemory<char> frame, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    {
        ObjectDisposedException.ThrowIf(this.disposed, "This session has been disposed.");
    }
    ...
    private static JsonRewriteOptions MapOptions(JsonKernelOptions? runtime, RuleInfo[] rules)
    {
        ...
        return new()
        {
            ...
            OnRuleMetrics = runtime.OnRuleMetrics is null
                ? null
                : stats => runtime.OnRuleMetrics(Project(stats, ruleNames, ruleGroups)),
