...
public sealed class TextSession<THandlers> : IDisposable, IAsyncDisposable
{
    ...
    private static TextRewriteOptions MapOptions(
        TextRuntimeOptions? runtime,
        RuleInfo[] rules,
        TextSession<THandlers> session)
    {
        runtime ??= new();
    ...
    private static TextMatchContext ToContext(MatchContext context, RuleInfo[] rules)
        => new(new(context.RuleId, rules[context.RuleId].Name, rules[context.RuleId].Group), context.MatchLength, context.Metrics);
