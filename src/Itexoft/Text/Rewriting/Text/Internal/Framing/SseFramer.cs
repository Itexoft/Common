...
internal sealed class SseFramer : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await this.EmitEventsAsync(emit, cancellationToken).ConfigureAwait(false);
        this.CheckTailSize();
    }
    ...
    public ValueTask FlushPendingAsync(
        Func<ReadOnlyMemory<char>, ReadOnlyMemory<char>, CancellationToken, ValueTask> emit,
        CancellationToken cancellationToken)
    {
        if (this.length == 0)
        ...
        return emit(this.buffer.AsMemory(0, this.length), ReadOnlyMemory<char>.Empty, cancellationToken);
    }
    ...
    private async ValueTask EmitEventsAsync(
        Func<ReadOnlyMemory<char>, ReadOnlyMemory<char>, CancellationToken, ValueTask> emit,
        CancellationToken cancellationToken)
    {
        var searchStart = 0;
        ...
        while (true)
        {
            ...
            await emit(this.buffer.AsMemory(0, payloadLen), delimiterMemory, cancellationToken).ConfigureAwait(false);
