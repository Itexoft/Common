...
internal sealed class OutputSink : IFlushableTextSink, IDisposable
{
    ...
    public OutputSink(
        ITextSink inner,
        TextRewriteOptions options,
        Func<RewriteMetrics> metricsProvider,
        ArrayPool<char> pool)
    {
        this.inner = inner;
        ...
        if (!string.IsNullOrEmpty(options.SseDelimiter))
            this.sseFramer = new(
                options.SseDelimiter!,
                options.SseIncludeDelimiter,
                options.SseMaxEventSize,
                options.SseOverflowBehavior,
                pool);
    }

    public void Dispose()
    {
        this.sseFramer?.Dispose();
    }
    ...
    public async ValueTask WriteAsync(ReadOnlyMemory<char> buffer, CancellationToken cancellationToken)
    {
        if (this.framing is not null)
        {
            this.AppendFrameBuffer(buffer.Span);
            await this.EmitFramedAsync(this.EmitAsync, cancellationToken).ConfigureAwait(false);
        ...
        if (this.sseFramer is not null)
        {
            await this.sseFramer.ProcessAsync(buffer, this.EmitAsync, cancellationToken).ConfigureAwait(false);

            return;
        }

        await this.EmitAsync(buffer, ReadOnlyMemory<char>.Empty, cancellationToken).ConfigureAwait(false);
    }
    ...
    public ValueTask FlushPendingAsync(CancellationToken cancellationToken)
    {
        if (this.framingBuffer is not null && this.framingBuffer.Length != 0)
        {
            ...
            return this.EmitAsync(pending.AsMemory(), ReadOnlyMemory<char>.Empty, cancellationToken);
        }

        if (this.sseFramer is not null)
            return this.sseFramer.FlushPendingAsync(this.EmitAsync, cancellationToken);
    ...
    private async ValueTask EmitAsync(ReadOnlyMemory<char> span, ReadOnlyMemory<char> delimiter, CancellationToken cancellationToken)
    {
        ...
        if (filterAsync is not null)
        {
            ...
            if (transformed is null)
            {
                await this.inner.WriteAsync(span, cancellationToken).ConfigureAwait(false);
                if (!delimiter.IsEmpty)
                    await this.inner.WriteAsync(delimiter, cancellationToken).ConfigureAwait(false);

                return;
            }

            if (transformed.Length != 0)
                await this.inner.WriteAsync(transformed.AsMemory(), cancellationToken).ConfigureAwait(false);
        ...
        if (transformedSync.Length != 0)
            await this.inner.WriteAsync(transformedSync.AsMemory(), cancellationToken).ConfigureAwait(false);

        if (!delimiter.IsEmpty)
            await this.inner.WriteAsync(delimiter, cancellationToken).ConfigureAwait(false);
    }

    private void AppendFrameBuffer(ReadOnlySpan<char> span)
    {
        this.framingBuffer!.Append(span);
    }
    ...
    private async ValueTask EmitFramedAsync(
        Func<ReadOnlyMemory<char>, ReadOnlyMemory<char>, CancellationToken, ValueTask> emit,
        CancellationToken cancellationToken)
    {
        var text = this.framingBuffer!.ToString();
        var remaining = text.AsMemory();
        while (this.framing!.TryCutFrame(ref remaining, out var frame))
            await emit(frame, ReadOnlyMemory<char>.Empty, cancellationToken).ConfigureAwait(false);
