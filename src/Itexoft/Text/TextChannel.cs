...
public class TextChannel : IDisposable
{
    ...
    private sealed class BufferedTextWriter(Channel<char> channel) : TextWriter
    {
        ...
        public override Task WriteAsync(char[] buffer, int index, int count)
            => this.WriteAsync(buffer.AsMemory(index, count));

        public override Task WriteAsync(string? value)
            => value is null ? Task.CompletedTask : this.WriteAsync(value.AsMemory());
        ...
        private void WriteChar(char value)
        {
            ...
            this.writer.WriteAsync(value).AsTask().GetAwaiter().GetResult();
        }
    ...
    private sealed class BufferedTextReader(ChannelReader<char> reader) : TextReader
    {
        ...
        public override ValueTask<int> ReadAsync(Memory<char> buffer, CancellationToken cancellationToken = default)
            => this.ReadAsyncInternal(buffer, cancellationToken);

        private async ValueTask<int> ReadAsyncInternal(Memory<char> buffer, CancellationToken cancellationToken)
        {
            var written = 0;
            while (written < buffer.Length)
            {
