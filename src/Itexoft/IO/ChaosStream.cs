...
public sealed class ChaosStream : Stream
{
    ...
    public override bool CanRead => !this.disposed && this.InnerStream.CanRead;

    public override bool CanSeek => !this.disposed && this.InnerStream.CanSeek;

    public override bool CanWrite => !this.disposed && this.InnerStream.CanWrite;
    ...
    public async override ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        var read = await this.InnerStream.ReadAsync(buffer, cancellationToken).ConfigureAwait(false);
        if (read <= 0)
        {
            this.RecordOperation(ChaosOperation.Read, 0);
    ...
    public async override ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        if (inject && this.ShouldFlip(this.options.WriteBitFlipProbability, this.options.MaxWriteBitFlips, out var flips))
        {
            ...
            try
            {
                ...
                await this.InnerStream.WriteAsync(temp, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                ArrayPool<byte>.Shared.Return(rented);
        ...
        {
            await this.InnerStream.WriteAsync(memory, cancellationToken).ConfigureAwait(false);
        }
