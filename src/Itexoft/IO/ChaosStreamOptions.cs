...
public sealed class ChaosStreamOptions
{
    ...
    public Func<TimeSpan, CancellationToken, ValueTask>? DelayScheduler { get; set; }
    ...
    internal void Validate()
    {
        ...
        if (this.MaxReadTruncationBytes < 0)
            throw new ArgumentOutOfRangeException(
                nameof(this.MaxReadTruncationBytes),
                this.MaxReadTruncationBytes,
                "Value must be non-negative.");

        if (this.MaxWriteTruncationBytes < 0)
            throw new ArgumentOutOfRangeException(
                nameof(this.MaxWriteTruncationBytes),
                this.MaxWriteTruncationBytes,
                "Value must be non-negative.");
