...
internal sealed class FileTable
{
...
#if DEBUG
        DebugUtility.Log($"[FileTable] Allocate id={fileId.Value} kind={kind}");
#endif
        try
        {
...
#if DEBUG
            DebugUtility.Log(
                $"[FileTable] Update file={fileId.Value} length={updated.Length} extents={string.Join(",", updated.Extents.Select(e => $"[{e.Start.Value}..{e.EndExclusive})"))}");
#endif
            return updated;
        }
        finally
...
#if DEBUG
            if (removed)
                DebugUtility.Log($"[FileTable] Remove file={fileId.Value}");
#endif
            return removed;
        }
        finally
...
#if DEBUG
            DebugUtility.Log($"[FileTable] LoadFrom entries={this.index.Count}");
#endif
        }
        finally
        {
    ...
    private static FileAttributes NormalizeAttributes(FileKind kind, FileAttributes attributes)
    {
        return kind switch
        {
