...
internal sealed class CompactionEngine : IDisposable
{
        ...
        int pageSize)
    {
        ...
        this.bufferPool = new AlignedBufferPool(pageSize, Environment.ProcessorCount * 2);
    ...
    {
        if (Interlocked.CompareExchange(ref this.disposed, 0, 0) != 0)
            return;
    ...
    private bool CompactFile(FileId fileId, FileMetadata initialMetadata)
    {
        ...
        foreach (var oldExtent in metadata.Extents)
        {
            for (var offset = 0; offset < oldExtent.Length && remaining > 0; offset++)
            {
                ...
                this.storage.WritePage(new PageId(span.Start.Value + newPageIndex), buffer);
                newPageIndex++;
    ...
    private sealed class AlignedBufferPool : IDisposable
    {
        ...
        public AlignedBufferPool(int bufferSize, int capacity)
        {
        ...
        public AlignedBufferLease Lease()
        {
            if (!this.buffers.TryTake(out var holder))
            {
                ...
                {
                    holder = new BufferHolder(this.bufferSize);
                }
                else
                {
                    Interlocked.Decrement(ref this.created);
            ...
            return new AlignedBufferLease(this, holder);
        }
        ...
        internal unsafe sealed class BufferHolder : IDisposable
        {
            ...
            public Span<byte> Span
            {
                ...
                get
                {
                    ...
                    return new Span<byte>(this.pointer, this.size);
                }
            }
