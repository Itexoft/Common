...
public sealed class VirtualFileSystem : IDisposable
{
    ...
    private VirtualFileSystem(Stream baseStream, VirtualFileSystemOptions options)
    {
        ...
        SuperblockInspector.Inspect(
            baseStream,
            out _,
            out var detectedPageSize,
            out var hasValidSuperblock);

        var existingPageSize = hasValidSuperblock ? detectedPageSize : (int?)null;
        (this.pageSize, this.featureRegistry) = options.Materialize(existingPageSize);
        ...
        this.services.Add(this.baseStream);
        this.services.Add(this.storage);

        var featureContext = new FeatureContext(this.services);
        this.featureRegistry.AttachAll(featureContext);
        ...
        DebugUtility.Log(
            $"[VFS] Mounted stream length={this.baseStream.Length} pageSize={this.pageSize}, compaction={(options.EnableCompaction ? "on" : "off")}, mirror={(this.storage.IsMirrored ? "on" : "off")}");
    ...
    internal Stream UnderlyingStream => this.baseStream;
    internal int PageSize => this.pageSize;
    ...
    public Stream OpenFile(string path, FileMode mode, FileAccess access)
    {
        ...
        DebugUtility.Log($"[VFS] OpenFile path={path} mode={mode} access={access} fileId={fileId.Value} parentId={parentId.Value}");
        var lockHandle = (access & FileAccess.Write) != 0
            ? this.lockManager.AcquireExclusive(fileId)
            : this.lockManager.AcquireShared(fileId);

        var metadata = this.fileTable.Get(fileId);

        return new VirtualFileStream(
            this.storage,
            this.extentAllocator,
            this.fileTable,
            this.directoryIndex,
    ...
    internal string DescribeDebugUsage()
    {
        ...
        return
            $"pages: total={totalPages}, data={usedData}, metadata={usedMeta}, superblock={reservedSuperblock}, remaining={unused}, metaSpans={{File:{FormatSpan(metadataSnapshot.FileTable)},Dir:{FormatSpan(metadataSnapshot.DirectoryIndex)},Attr:{FormatSpan(metadataSnapshot.AttributeTable)}}}";
    }
    ...
    internal string DescribeDebugTracker(PageSpan span) => DebugPageTracker.Describe(this.storage, span);

    internal string DescribeDebugPage(long pageNumber) => DebugPageTracker.DescribePage(this.storage, pageNumber);
    ...
    public void CreateFile(string path, FileAttributes attributes = FileAttributes.Normal)
    {
        ...
        if (this.directoryIndex.TryGet(parent, name, out var existing))
        ...
        DebugUtility.Log($"[VFS] CreateFile path={path} parent={parent.Value} fileId={fileId.Value}");
    ...
    public IReadOnlyList<string> EnumerateDirectory(string path)
    {
        ...
        return this.directoryIndex.Enumerate(directoryId)
    ...
    private FileId CreateDirectoryInternal(FileId parent, string name)
    {
        ...
        DebugUtility.Log($"[VFS] CreateDirectoryInternal parent={parent.Value} name={name} id={directoryId.Value}");
        this.directoryIndex.Upsert(parent, name, entry);
    ...
    {
        DebugUtility.Log($"[VFS] DeleteFileEntry parent={parentId.Value} target={entry.TargetId.Value}");
        ...
        {
            DebugUtility.Log(
                $"[VFS] Freeing extents for file {entry.TargetId.Value}: {string.Join(", ", metadata.Extents.Select(e => $"[{e.Start.Value}..{e.EndExclusive})"))}");
    ...
    private static void SynchronizeMirrors(FileStream primary, FileStream mirror)
    {
        ...
        try
        {
            ...
            if (primaryValid && mirrorValid)
            {
                ...
                {
#if DEBUG
                    DebugUtility.Log($"[VFS] Detected drift between primary and mirror. prefer={(preferMirror ? "mirror" : "primary")}");
#endif
                    if (preferMirror)
                        CopyStreamContents(mirror, primary);
                    else
