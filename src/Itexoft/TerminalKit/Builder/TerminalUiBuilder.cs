...
/// </summary>
internal sealed class TerminalUiBuilder<TScreen>
    where TScreen : TerminalComponentDefinition
{
    private readonly TerminalBuildContext _context = new();
    private readonly List<TerminalInputBinding> _input = [];
    private readonly TerminalNode _root;
    private readonly List<TerminalStateSlice> _state = [];

    private TerminalUiBuilder()
    {
        this._root = TerminalNode.Create(typeof(TScreen));
        this._context.Register(this._root);
    }
    ...
    public TerminalUiBuilder<TScreen> Configure(Action<TerminalComponentBuilder<TScreen>> configure)
    {
        ...
        configure(new(this._root, this._context));
    ...
    public StateHandle<TState> WithState<TState>(string name, TState state)
    {
        ...
        this._state.RemoveAll(slice => string.Equals(slice.Name, name, StringComparison.Ordinal));
        this._state.Add(new(name, state));
    ...
    {
        this._input.Add(new(mode, gesture, action));
    ...
    public TerminalUiBuilder<TScreen> BindEvent<TComponent>(
        TerminalComponentHandle<TComponent> handle,
        TerminalEventKey eventKey,
        string handler)
        where TComponent : TerminalComponentDefinition
    {
        ...
        var node = this._context.Resolve(handle.Id);
        node.AddEvent(new(eventKey, handler));

        return this;
    }

    public TerminalSnapshot BuildSnapshot() => new(this._root, this._state.ToArray(), this._input.ToArray());
