...
{
    private readonly Dictionary<string, TerminalNode> _nodeIndex = new(StringComparer.Ordinal);
    private readonly Dictionary<string, object?> _stateIndex = new(StringComparer.Ordinal);
    ...
    public TerminalNode? FindNode(string id)
    {
        ...
        return this._nodeIndex.TryGetValue(id, out var node) ? node : null;
    }
        ...
        where TComponent : TerminalComponentDefinition
    {
        ...
        return this._nodeIndex.TryGetValue(handle.Id, out node);
    }

    public bool TryResolveState<TState>(StateHandle<TState> handle, out TState state)
    {
        if (this._stateIndex.TryGetValue(handle.Name, out var value) && value is TState typed)
        {
            state = typed;
    ...
    public object? GetStateSlice(string name) => this._stateIndex.TryGetValue(name, out var value) ? value : null;
    ...
    private void IndexState(IReadOnlyList<TerminalStateSlice> slices)
    {
        foreach (var slice in slices)
            this._stateIndex[slice.Name] = slice.Value;
    }
}