...
{
    private readonly List<TerminalPendingEntryLoad> _pendingLoads = [];
    private readonly Dictionary<PropertyInfo, TerminalPropertyValueState> _propertyStates = new();
    ...
    private void BuildCollectionItemEntries()
    {
        if (this.Target is IDictionary dictionary)
        {
            ...
            foreach (var entry in EnumerateDictionaryEntries(dictionary))
            {
                ...
                this._pendingLoads.Add(new(null, entry.Value, 0, this.Entries.Count - 1));
                dictIndex++;
            }
        ...
        foreach (var item in enumerable)
        {
            ...
            this._pendingLoads.Add(new(null, item, 0, this.Entries.Count - 1));
            index++;
        }
    }
    ...
    private void BuildObjectEntries()
    {
        ...
        foreach (var property in properties)
        {
            ...
            var typeName = state.AllowStale && !string.IsNullOrWhiteSpace(state.TypeName)
                ? state.TypeName!
                : string.Empty;
            ...
            this._pendingLoads.Add(new(property, null, version, entryIndex));
        }
    ...
    private static IEnumerable<DictionaryEntry> EnumerateDictionaryEntries(IDictionary dictionary)
    {
        ...
        return snapshot
            .OrderBy(entry => FormatDictionaryKey(entry.Key), StringComparer.OrdinalIgnoreCase);
    }
    ...
    internal bool TryGetPropertyState(PropertyInfo property, [NotNullWhen(true)] out TerminalPropertyValueState? state) =>
        this._propertyStates.TryGetValue(property, out state);

    internal IEnumerable<TerminalPendingEntryLoad> ConsumePendingLoads(Func<TerminalPendingEntryLoad, bool>? predicate = null)
    {
        if (this._pendingLoads.Count == 0)
            return [];

        var snapshot = predicate == null
            ? this._pendingLoads.ToArray()
            : this._pendingLoads.Where(predicate).ToArray();

        if (snapshot.Length == 0)
            return [];

        foreach (var item in snapshot)
            this._pendingLoads.Remove(item);
    ...
    {
        if (!this._propertyStates.TryGetValue(property, out var state))
        {
            state = new();
            this._propertyStates[property] = state;
        }
