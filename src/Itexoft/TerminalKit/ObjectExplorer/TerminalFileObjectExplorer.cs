...
/// </summary>
public sealed class TerminalFileObjectExplorer<T> : TerminalObjectExplorer
    where T : class
{
    ...
    public TerminalFileObjectExplorer(string filePath)
        : this(filePath, ResolvePath(filePath)) { }

    private TerminalFileObjectExplorer(string originalPath, string resolvedPath)
        : base(null, BuildTitle(resolvedPath))
    {
        this._filePath = resolvedPath;
        this._serializerOptions = this.CreateSerializerOptions();
    ...
    public void Save(Action<Exception>? onException = null)
    {
        ...
        try
        {
            ...
            using var stream = File.Create(this._filePath);
            JsonSerializer.Serialize(stream, typed, this._serializerOptions);
            stream.Flush();
        }
        catch (Exception ex)
        {
    ...
    private T? LoadValue(string fullPath)
    {
        ...
        try
        {
            ...
            var value = JsonSerializer.Deserialize<T>(stream, this._serializerOptions);
    ...
    private static string ResolvePath(string path)
    {
        ...
        var baseDirectory = !string.IsNullOrWhiteSpace(entry)
