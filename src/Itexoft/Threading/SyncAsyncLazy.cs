...
internal struct SyncAsyncLazy<T>
{
    ...
    private readonly Func<CancellationToken, ValueTask<T>>? asyncFactory;
    private T value;
    ...
    private SyncAsyncLazy(Func<T>? syncFactory, Func<CancellationToken, ValueTask<T>>? asyncFactory)
    {
    ...
    public static SyncAsyncLazy<T> Create(Func<T>? syncFactory, Func<CancellationToken, ValueTask<T>>? asyncFactory = null) =>
    ...
    /// </summary>
    public static SyncAsyncLazy<T> CreateOrDefault(
        Func<T>? syncFactory,
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ValueTask<T> GetOrCreateAsync(CancellationToken cancellationToken = default)
    {
        if (this.hasValue)
        ...
        return this.GetOrCreateAsyncCore(cancellationToken);
    }

    private async ValueTask<T> GetOrCreateAsyncCore(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();

        var result = await this.asyncFactory!(cancellationToken).ConfigureAwait(false);
        this.value = result;
        this.hasValue = true;
