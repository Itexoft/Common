...
internal static class EmbeddedArchiveLoader
{
    ...
    {
        await using var archive = new ZipArchive(stream, ZipArchiveMode.Read, false, Encoding.UTF8);
...
{
    private readonly Dictionary<string, EmbeddedStaticFile> _files = files ?? throw new ArgumentNullException(nameof(files));
