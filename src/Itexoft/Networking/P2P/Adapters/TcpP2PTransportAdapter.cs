...
/// <summary>
/// TCP adapter: opens separate sockets for data and control channels.
/// </summary>
public sealed class TcpP2PTransportAdapter(TcpP2PTransportAdapterOptions? options = null) : IP2PTransportAdapter
{
    private readonly TcpP2PTransportAdapterOptions _options = options ?? new TcpP2PTransportAdapterOptions();
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(endpoint.Transport, P2PTransportNames.Tcp, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Endpoint transport '{endpoint.Transport}' is not supported by the TCP adapter.");
...
{
    /// <summary>
    /// TCP_NODELAY toggle (true by default).
    /// </summary>
    public bool? NoDelay { get; set; } = true;

    /// <summary>
    /// Whether to enable TLS for the control channel when the data stream is encrypted.
    /// </summary>
    public bool EnableControlChannelTls { get; set; } = true;

    /// <summary>
    /// TLS options factory (bool parameter indicates control vs data channel).
    /// </summary>
    ...
    /// <summary>
    /// Additional socket configuration hook after connect.
    /// </summary>
    public Action<Socket>? ConfigureSocket { get; set; }

    /// <summary>
    /// Send buffer size (default from OS).
    /// </summary>
    public int? SendBufferSize { get; set; }

    /// <summary>
    /// Receive buffer size (default from OS).
    /// </summary>
    public int? ReceiveBufferSize { get; set; }

    /// <summary>
    /// Whether to enable TCP keep-alive (enabled by default).
    /// </summary>
    ...
    /// <summary>
    /// Keep-alive retry count before the connection is considered dead (if supported).
    /// </summary>
