...
public sealed class P2PConnectionManager : IP2PConnectionManager
{
    ...
    public P2PConnectionHandle? CurrentConnection { get; private set; }

    public P2PConnectionState State { get; private set; } = P2PConnectionState.Disconnected;
    ...
    public async Task DisconnectAsync(DisconnectReason reason = DisconnectReason.Manual, CancellationToken cancellationToken = default)
    {
        ...
        lock (this._stateLock)
        {
            ...
            handleToDispose = this.CurrentConnection;
    ...
    public P2PConnectionMetrics GetMetrics()
    {
        ...
        return new()
        {
            ...
            LastEndpointLabel = this.CurrentConnection?.Endpoint.Label,
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
            try
            {
                ...
                this.ChangeState(this.State, P2PConnectionTransitionCause.DialSucceeded, connection.RemoteEndPoint?.ToString());
            ...
            catch (RetryAfterException retryAfter)
            {
                ...
                this.ChangeState(this.State, P2PConnectionTransitionCause.DialFailed, $"{endpoint}: retry-after {retryAfter.Delay}");
            ...
            catch (Exception ex)
            {
                ...
                this.ChangeState(this.State, P2PConnectionTransitionCause.DialFailed, $"{endpoint}: {ex.Message}");
        ...
        TokenSourceKind tokenKind)
    {
        ...
        lock (this._stateLock)
        {
            ...
            previousHandle = this.CurrentConnection;
            previousCts = this._livenessCts;

            this._transportConnection = connection;
            this.CurrentConnection = newHandle;
    ...
    private void ChangeState(P2PConnectionState newState, P2PConnectionTransitionCause cause, string? evidence)
    {
        ...
        lock (this._stateLock)
        {
            if (this.State == P2PConnectionState.Disposed)
                return;
    ...
    {
        var endpoint = this.CurrentConnection?.Endpoint;
    ...
    private void BroadcastEndpointPenalty(string endpointKey, FailureSeverity severity, TimeSpan duration, bool fromRetryAfter)
    {
        ...
        var handle = this.CurrentConnection;
...
file sealed class FaultAwareStream(Stream inner, Func<Exception, Task> faultCallback, Action? activityCallback = null)
    : Stream
{
    private readonly Func<Exception, Task> _faultCallback = faultCallback ?? throw new ArgumentNullException(nameof(faultCallback));
