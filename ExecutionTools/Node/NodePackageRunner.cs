...
public abstract class NodePackageRunner : ToolRunner
{
    protected readonly Options options;

    protected NodePackageRunner(string executablePath, string workingDirectory, Options? options = null) : base(executablePath, workingDirectory)
    {
    ...
    protected virtual async Task<RunResult> ExecuteAsync(List<string> args, CancellationToken token)
    {
        ...
        return await this.RunAsync(args, token);
    }
    ...
    : NodePackageRunner(yarnPath, workingDirectory, options)
{
    ...
    protected override async Task<RunResult> ExecuteAsync(List<string> args, CancellationToken token)
    {
        if (this.options.Verbose)
            args.Add("--verbose");
        if (this.options.Force)
